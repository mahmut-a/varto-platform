...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    const item = await orderExtService.updateVartoOrderItems({
...
export const DELETE = async (req: MedusaRequest, res: MedusaResponse) => {
    const orderExtService: OrderExtensionModuleService = req.scope.resolve(ORDER_EXTENSION_MODULE)
    await orderExtService.deleteVartoOrderItems(req.params.itemId)
    res.status(200).json({ id: req.params.itemId, deleted: true })
}
